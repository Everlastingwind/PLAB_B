...
def _purchase_history_for_hero(
    ...
    *,
    ...
    """
    时间轴用购买流水：比赛内（time>=0）每条 DOTA_COMBATLOG_PURCHASE。
    仅排除空 key / 配方；与前端 buildPurchaseEventsForTime 对齐字段 item + time。
    """
    out: List[Dict[str, Any]] = []
    for e in events:
        if e.get("type") != "DOTA_COMBATLOG_PURCHASE":
...
def main() -> None:
    ...
    # 上传到 Supabase（剔除不入库字段）
    upload_data = copy.deepcopy(slim)
    for key in ("_meta", "match_source", "match_tier"):
